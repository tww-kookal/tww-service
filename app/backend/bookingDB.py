...
def queryAvailableRoomsDB(check_in, check_out, number_of_people, room_id, conn):
    try:
...
def listBookingsSinceDB(startingDate: date, is_check_in_date: bool = True, conn = None):
    try:
        cursor = conn.cursor(dictionary=True)
        query = """
...
def guestsForDay(forDate: date, conn):
    try:
        cursor = conn.cursor()
...
def getBookingById(booking_id : int, conn):
    try:
        cursor = conn.cursor(dictionary=True)
        query = """
            SELECT b.booking_id, b.customer_id, CONCAT(c.first_name, ' ', c.last_name) as "customer_name", 
...
def updateBookingDB(booking: dict, is_same_room : bool = False, conn = None):
    try:
        cursor = conn.cursor()
        query = """
            UPDATE bookings 
...
def persistBookingDB(booking: dict, conn):
    try:
        cursor = conn.cursor()
        query = """
