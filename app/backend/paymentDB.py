...
def queryPaymentsForBooking (booking_id : int, conn):
    try:
...
def persistPaymentDB(payment: dict, conn):
    try:
        cursor = conn.cursor()
        query = """
...
def updatePaymentDB(payment: dict, conn):
    try:
        cursor = conn.cursor()
        query = """
...
def deletePaymentDB(paymentId: int, conn):
    try:
        cursor = conn.cursor()
        query = """
